...
export function buildCorridor(scene) {
  ...
  const floorMat = new THREE.MeshStandardMaterial({ map: themeTex[0].floorTex, roughness: 0.9 });
...
export function setupLighting(scene) {
  const ambient = new THREE.AmbientLight(THEMES[0].ambient, 3.2);
  scene.add(ambient);
  ...
  return { ambient, key, rim, setTheme };
}
